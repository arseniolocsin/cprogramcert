...
void SRT__copyElement(char * source, char * dest, size_t element_size)
{
					...
					)			  
{
	if(num_elements > 1)
	{
		...
		size_t highIndex = 0;
		size_t tempIndex = 0;
		size_t lowNumElements;
		size_t highNumElements;
		char * lowArray;
		char * highArray;
		char * tempArray;
		int compareResult;
			...
			tempIndex++)
		{
			...
			if(compareResult < 0)
			{
				SRT__copyElement( OFFSETVALUE(lowArray, lowIndex, element_size), 
			...
			{
				SRT__copyElement( OFFSETVALUE(highArray, highIndex, element_size),
								  OFFSETVALUE(tempArray, tempIndex, element_size),
								  element_size
								);
				highIndex++;
		...
		{
			SRT__copyElement( OFFSETVALUE(tempArray, tempIndex, element_size), 
